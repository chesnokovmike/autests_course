...
def everything_for_your_cat(cats_data):
    """
    Оптимизация хранения данных по котикам и их владельцам
    :param cats_data: - список котиков и владельцев
    :return our_str: - строка из имени владельца и его котиков
    """
    our_str = ''
    beauty_dict = dict()
    for entry in cats_data:
        cat_record = entry[0] + ', ' + str(entry[1])
        beauty_dict.setdefault(entry[2:], []).append(cat_record)
    for owner, cat in beauty_dict.items():
        our_str += ' '.join(owner) + ': ' + '; '.join(cat) + '\n'
    return our_str
