...
class Segment:
    ...
    def length(self):
        ...
        """
        return round(math.sqrt((self.second_point[0] - self.first_point[0]) ** 2 +
                               (self.second_point[1] - self.first_point[1]) ** 2), 2)

    def x_axis_intersection(self):
        """
        Пересекает ли отрезок ось oX
        :return True: если отрезок пересекает ось
        False: если отрезок не пересекает ось
        """
