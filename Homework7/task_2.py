...
class PersonInfo:
    def __init__(self, name, age, *department):
        """
        Экземпляр класса "Сотрудник"
        :param name: имя и фамилия сотрудника
        :param age: возраст
        :param department: подразделения от головного до подразделения сотрудника
        """
        self.name = name
        self.age = age
    ...
    def short_name(self):
        ...
        :return: возвращает фамилию и имя сотрудника в формате [Фамилия И.]
        """
        space_position = self.name.index(' ')
        return self.name[space_position + 1:] + ' ' + self.name[0] + '.'

    def path_deps(self):
        """
        Путь до подразделения сотрудника
        :return: возвращает путь до подразделения сотрудника разделенный "-->"
        """
        return ' --> '.join(self.department)

    def new_salary(self):
        """
        Возвращает зарплату сотрудника после индексации
        :return new_salary: новая зарплата сотрудника
        """
        letter_dict = collections.Counter()
        const = 0
        for department in self.department:
            for sym in department:
                letter_dict[sym] += 1
        letter_dict = letter_dict.most_common(3)
